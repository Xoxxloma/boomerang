 ...
 * довезёт). Возвращает суммарный результат или null (пусто / уже флашится / уже обработано).
 */
export async function flushBurst(api: Api, userId: number): Promise<BatchResult | null> {
  ...
  try {
    ...
    for (;;) {
      ...
      // Если batchIngest бросит — буфер и сессия остаются, ничего не потеряно (ретрай довезёт).
